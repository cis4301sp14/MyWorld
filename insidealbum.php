...
<div class="container" style="margin-top:50px"><table align="center"> 
 ...
 <script type="text/javascript">
	<?php 
	 ...
	 $albuminfo = pg_query($db, "select albumid, userid from albums where albumname = '$albumname'");
	 $album_id = pg_fetch_result($frdinfo,0,0);
	 $auserid = pg_fetch_result($frdinfo,0,1);
	 echo $album_id."!!!!!!!!!!!!!!";
	 $picture = pg_query($db, "select photoname, lat, lon, photoid from photo where albumid = $album_id");
 	  $row=0; 
 	  while($pic = pg_fetch_assoc($picture)){ 
		  $location[$row]=$pic['lat'].", ".$pic['lon'];
		  $photoid[$row++]=$pic['photoid'];
	  }
	?>
		...
		google.maps.event.addListener(marker$i, 'click', (function(marker, index) {
            return function() {
              infowindow.setContent(\"$photoid[$i]\");
