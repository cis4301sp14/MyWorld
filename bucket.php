...
<?php  
	...
	if ($_SESSION['user'] == '') {
	 ...
	 exit;
		...
		$resultTF = pg_query($dbconn, "select EXISTS (select * from bucket where photoid=$pid and userid = $user_id)::int");
		$result = pg_fetch_result($resultTF,0,0);
		 
		 if(!$result){
			$result = pg_query($dbconn, "insert into bucket values ($pid,false,$user_id)");

			echo "This picture has been successfully added to your bucket.";
		 }else{
			$result = pg_query($dbconn, "delete from bucket where photoid=$pid and userid = $user_id");
